...
void Day_08_Part_2()
{
	...
	int networkGlobal = -1;
	int cptMax = 0;
	boxCouple_t lastBoxCouple{ -1,-1,-1 };
	for (auto& box : sortedClosestBoxes)
	{
		...
		else if (boxes[box.indexBox1].network == boxes[box.indexBox2].network)
			continue;
		else if (boxes[box.indexBox1].network != -1)
		{
			...
			lastBoxCouple.indexBox1 = box.indexBox1;
			lastBoxCouple.indexBox2 = box.indexBox2;
		}
	...
	auto back = sortedClosestBoxes.end();

	println("---");
	println(boxes[lastBoxCouple.indexBox1].toString());
	println(boxes[lastBoxCouple.indexBox2].toString());
	result = boxes[lastBoxCouple.indexBox1].point.x * boxes[lastBoxCouple.indexBox2].point.x;
